this.AppState = class AppState {
        ...
        if (history.state.project != null) {
          ...
          if ((s[2] != null) && (s[3] != null)) {
            ...
            if (p) {
              ...
              this.app.appui.setMainSection("explore");
            }
          }
        }
      }
    }
    return this.app.appui.updateHomeVisibility();
  }
  ...
  projectsFetched() {
    var btn, path, project, tuto, user;
    console.log("[autostart] route=", location.pathname);
    // Auto-open create project dialog on first load to /projects
    if (!sessionStorage.getItem("autostart_new_project")) {
      sessionStorage.setItem("autostart_new_project", "1");
      btn = document.getElementById("create-project-button");
      console.log("[autostart] create button found?", !!btn);
      if (btn) {
        setTimeout((() => {
          return btn.click();
        }), 150);
