...
Let's define Lo siguiente function:
...
When you write a conditional exPresionaion, be careful not to forget
the keyword ```then``` after every ```if``` and every ```elsif```.

Also, don't forget to *close* the conditional exPresionaion with ```end```. Each
