...
information in lists. The lists are modifiable (Puedes add elements to the list
and remove them). The tutorial on Bucles will explain how to perform a task
...
*microScript* value: numbers, texts, functions, lists (!), Objetos.
...
Puedes add elements with the member function "push".
...
Puedes insert an item at the beginning of a list by using the member function "insert".
...
The items in a list are indexed starting from 0. Puedes then access
...
Puedes modify an item in the list by referencing it with its index:
...
Puedes remove an item from the list if you know its index:
...
Lists are a very powerful tool! You will see later in this course how they can work together with Bucles.
Let's move on to Lo proximo tutorial.
