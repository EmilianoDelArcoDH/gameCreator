...
is a location in La computadora's memory that contains a value and which
is designated by an identifier chosen by the programmer.

During this tutorial, do not hesitate to copy the various examples in La consola
...
There are a few rules for naming your variables correctly. In microScript, Deberias
only use letters of the alphabet, without any accent. Puedes usar them in upper or lower case.
...
2_players = 2 // Puedes't start with a number!
...
* Some text, which we will call the "string" Escribe (because it is a *string of characters*). Example: 'Hello',
"You win!"...
...
A variable can be used in any *exPresionaion* of the program and will then be
...
In the example above, the "print" function displays in La consola the value
...
We will see during the study of functions and Objetos that some variables can have
