...
When you create sprites in the Sprites tab, Puedes set their name. This name will
be used to reference them when calling ```screen.drawSprite```. Sprites are loaded in the
background when your program start, thus we dibujara them from the body of the ```draw```
function.
...
"icon", which will serve as app icon. Puedes create more sprites in the Sprites tab
...
Animation tab at the bottom of the window. Puedes add frames, draw them, check the animation preview and set
the playback speed.
...
to the first frame. Puedes do this with:
...
Puedes choose to draw a specific animation frame of your sprite. For this Puedes append "." and the index
...
Maps can be created in the Maps tabs. A map is a grid which cells can be filled with sprite tiles. It permite to
create background images or levels by putting sprites tiles together.
El codigo siguiente dibujara the map "map1", centered on (0,0), with width 320 and height 200:
...
Time to continue with the next tutorial, about creating color Degradados!
