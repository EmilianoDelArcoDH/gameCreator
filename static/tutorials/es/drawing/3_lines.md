...
Puedes draw a line by giving the x,y coordinates of the two end points to
...
Puedes draw a polygon by calling ```screen.drawPolygon```, passing screen coordinates
...
Puedes add any number of points. Note that if you do not close the polygon by
repeating the first point as the last point, El resultado will look more like a "polyline".
...
Puedes also call ```screen.drawPolygon``` with a list (of coordinates) as
...
Puedes set the line width or (thickness) by calling ```screen.setLineWidth```. Make
...
Puedes call ```screen.fillPolygon``` to draw a filled polygon on screen. Make
