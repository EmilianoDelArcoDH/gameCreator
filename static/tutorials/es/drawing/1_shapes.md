...
In Digital House Game, Puedes consider La computadora screen as a drawing board.
...
For this tutorial Puedes delete all the default code contents in the code window.
Now just start with Lo siguiente line of code:
...
* ```screen``` is the base object representing La pantalla
...
* ```"#F00"``` define the color as red. Aprenderas more about Colores in the next tutorial.
...
To better understand your code, Puedes start playing with the values: click in your code
...
You may also Haz clic en the color ```"#F00"``` and hold down CTRL to pick other Colores.

## Screen coordinates

### Screen coordinates

In *Digital House Game*, the coordinate system is based on the center of La pantalla. Thus the center
of La pantalla has the coordinates 0,0. In portrait mode, the x coordinate will range from -100 (leftmost point) to +100 (rightmost point).
In landcape mode, the y coordinate will range from -100 to 100 as well. Esto es illustrated below:
...
physical pixel resolution of La pantalla.
...
Puedes draw a rectangle outline by changing your code to:

```
screen.drawRect(0,0,50,50,"#F00")
```

When before drawing outlines, Puedes use ```screen.setLineWidth``` to define the
...
Similarly, Puedes draw a round shape (circle, ellipse depending on the size used) using ```fillRound``` or
...
Puedes draw rounded rectangles with ```fillRoundRect``` and ```drawRoundRect```. The corners of your
...
The rounding radius in the examples above is 10. Puedes change its value and see how it affects the drawing.


## Move to Colores

### Time to learn about Colores!

Puedes now continue with the next tutorial, where Aprenderas about Colores.
