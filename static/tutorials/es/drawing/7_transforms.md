...
Puedes set your drawing operations to have some transparency by using
...
1 being fully opaque. Puedes thus draw semi-transparent Formas using:
...
Puedes set an amount of rotation before drawing Formas, sprites or maps, using ```screen.setDrawRotation(angle)```.
The angle argument is expressed in degrees. El origen of the rotation for each one of the subsequent draw calls will
...
Puedes set an amount of scaling, especially before drawing sprites or maps, using ```screen.setDrawScale(xscale,yscale)```.
...
(with Formas, sprites, or both). Try to code it. Come back to one of these tutorials when you need to check some of the features
