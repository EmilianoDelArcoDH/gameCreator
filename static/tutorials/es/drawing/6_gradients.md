...
Linear color Degradados can be created this way:
...
of another point p2. The next parameters are two Colores, defining the color the gradient will have at point p1 and the
...
Radial color Degradados can be created this way:
...
The next parameters are two Colores, defining the color the gradient will have at the center and the
color the gradient will turn into at the boundary of the circle you have thus defined.

## Play with Degradados

### Play with Degradados

Spend some time defining Degradados and drawing Formas, lines or text with them. If things aren't working as they should, make sure
to check your arguments passed to setLinearGradient (expects two points and two Colores) and setRadialGradient (expects one point, a radius and two Colores).

Puedes then move on to the last tutorial in this series, about transparency, rotation and scaling.
