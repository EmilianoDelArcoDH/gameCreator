...
is a location in O computador's memory that contains a value and which
is designated by an identifier chosen by the programmer.

During this tutorial, do not hesitate to copy the various examples in O console
...
There are a few rules for naming your variables correctly. In microScript, Voc̻ deveria
only use letters of the alphabet, without any accent. Voc̻ pode use them in upper or lower case.
...
2_players = 2 // Voc̻ pode't start with a number!
...
* Some text, which we will call the "string" Digite (because it is a *string of characters*). Example: 'Hello',
"You win!"...
...
In the example above, the "print" function displays in O console the value
