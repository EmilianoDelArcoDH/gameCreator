...
The ```print``` function writes to O console the value passed to it as an argument.
...
Here's how to define a function yourself that Voc̻ pode use afterwards
...
Our function "sumSquares" returns the sum of the squared values of its two arguments. Vamos tentar this:
...
We have just displayed the global context, i.e. the content of O computador's memory.
We can see there all the variables we have defined, in particular the functions that we
have created Neste tutorial.
