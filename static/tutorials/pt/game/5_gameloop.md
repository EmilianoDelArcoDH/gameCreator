...
We have also added some sounds that are produced by calling function ```audio.beep()```. Voc̻ pode check that too in the
code below.

Voc̻ pode also start adding more features to the game. Why not some flying Objetos that would need to be avoided
