...
We need our hero to jump on a tap or mouse Clique em the screen. Jumping means acquiring
...
Let's add O seguinte code to the ```update``` function body:
...
Now when you FaÃ§a seu herÃ³i pular, he goes through the roof and disappears real quick! That is because we haven't
introduced gravity yet. Gravity is similar to a downwards acceleration. Every short period of time, it decreases our
vertical velocity by a fixed amount. Let's add O seguinte line to our ```update``` function body:
