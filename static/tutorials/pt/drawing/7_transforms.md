...
Voc̻ pode set your drawing operations to have some transparency by using
...
1 being fully opaque. Voc̻ pode thus draw semi-transparent Formas using:
...
Voc̻ pode set an amount of rotation before drawing Formas, sprites or maps, using ```screen.setDrawRotation(angle)```.
...
Voc̻ pode set an amount of scaling, especially before drawing sprites or maps, using ```screen.setDrawScale(xscale,yscale)```.
