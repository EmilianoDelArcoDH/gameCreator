...
Voc̻ pode draw text by calling ```screen.drawText```. As drawing text requires that
...
Voc̻ pode use a different font by calling ```screen.setFont```:
...
* when your program is already running, have a peek at variable ```global.fonts``` in O console ; it retains the full list of available fonts.
...
Voc̻ pode move on to the next tutorial, about drawing Sprites e mapas.
