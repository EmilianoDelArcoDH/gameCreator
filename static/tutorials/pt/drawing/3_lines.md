...
Voc̻ pode draw a line by giving the x,y coordinates of the two end points to
...
Voc̻ pode draw a polygon by calling ```screen.drawPolygon```, passing screen coordinates
...
Voc̻ pode add any number of points. Note that if you do not close the polygon by
...
Voc̻ pode also call ```screen.drawPolygon``` with a list (of coordinates) as
...
Voc̻ pode set the line width or (thickness) by calling ```screen.setLineWidth```. Make
...
Voc̻ pode call ```screen.fillPolygon``` to draw a filled polygon on screen. Make
